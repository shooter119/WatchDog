...
- **断网可用**：语音识别与语义解析都支持本机运行（设置页可切换/强制离线），火场没信号也能录；本地模型仅 31MB，下载快，名单热词照常生效
...
由一线消防员在工作之余开发，用了豆包语音识别和 DeepSeek 语义理解做语音解析，后端和 App 全部开源。断网时自动切换本机 sherpa-onnx 语音识别（31MB 中文模型，支持名单热词）与规则解析，火场无信号也能用。项目进展见 [STATUS.md](STATUS.md)。
