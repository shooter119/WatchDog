...
| 模块 | 状态 |
| --- | --- |
| 后端 | ✅ Express；生产使用 CloudBase PG/PostgREST，本地测试保留 node:sqlite；警情 UUID 主键、事件流、参战力量、实名审计、版本冲突和离线补传已实现 |
...
backend/
  ...
  src/db.js              数据库适配入口：CloudBase PG/PostgREST 生产驱动 + SQLite 本地测试驱动
  migrations/            CloudBase PostgreSQL 版本化结构迁移
